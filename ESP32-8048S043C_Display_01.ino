/*
ESP32-8048S043C ARDUINO IDE 2.3.3 ESP32-S3, 8M PSRAM 16M Flash 4.3" 800x480
*/

#include <LovyanGFX.hpp>  // by lovyan03 ver 1.1.12
#include <lgfx_user/LGFX_ESP32S3_RGB_ESP32-8048S043.h>
#include <LGFX_TFT_eSPI.h>  // wrapper to make LovyanGFX behave like TFT_eSPI.
...
void setup() {
  ...
  for (int i = 0; i < 16; i++) {
    spr.setPaletteColor(i, palette[i]);
  }
...
void loop() {
  ...
  if (timeClient.isTimeSet()) {
    spr.drawString(timeClient.getFormattedTime(), 180, 155, 8);
    spr.drawString(getFormattedDate(), 140, 240, 8);
    ...
    if (data_age_min < 70) spr.drawString(outputString, 250, 330, 4);
